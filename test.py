...
def coo_to_csr(row, col, data, shape):
    n = len(data)
    inptr = torch.zeros(shape[0] + 1).to(torch.int32)
    indice = torch.zeros(n).to(torch.int32)
...
if __name__ == '__main__':
    ...
    row = torch.Tensor([0,0,1,1,2,4,4]).to(torch.int32)
    col = torch.Tensor([0,1,1,2,3,2,3]).to(torch.int32)
    data = torch.Tensor([6,2,3,4,5,7,1]).to(torch.int32)
    shape = [5,5]
    indices ,indptr = coo_to_csr(row, col, data, shape)
